...
Icons.Load = function(Name, ImageObj)
    local Asset = GetIcon(Name)

    if Asset and ImageObj then
        ImageObj.Image = Asset
    end
...
local script = G2L["4"]
    local ts = game:GetService("TweenService")
    local inTI = TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
    local outTI = TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.In)

    local NotifBind = script.Parent:WaitForChild("GSYIDFBUEJS")
    local NotificationUI = script.Parent:WaitForChild("Notification")
    local NotifTitle = NotificationUI:WaitForChild("Title")
    local NotifDescription = NotificationUI:WaitForChild("Description")

    local showPos = UDim2.new(0.87858, 0, 0.91987, 0)
    local hidePos = UDim2.new(1.2, 0, 0.91987, 0)

    NotificationUI.Position = hidePos
    ...
    local function processQueue()
        ...
        NotifDescription.Text = data.Description

        ts:Create(NotificationUI, inTI, {Position = showPos}):Play()
        task.wait(data.Duration)
        ts:Create(NotificationUI, outTI, {Position = hidePos}):Play()
        task.wait(0.4)
    ...
    NotifBind.Event:Connect(function(Title, Description, Duration)
        table.insert(queue, {
...
local script = G2L["2c"]
    ...
    task.spawn(function()
        ...
        if not isfile or not isfile("MonkeysStorage/cursor.png") then
            return
        end
